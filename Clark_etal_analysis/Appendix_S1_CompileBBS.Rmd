...
Specify the years to gather BBS data (note, crashes can sometimes occur when using multiple years). We found it is best to run the below functions separately for each year and save as the resulting `.Rdata` file. You can then bring all saved files in as a list and compile them at once (see below for methods to filter and compile)
```{r eval = FALSE}
year <- 2003
```

Gather the full dataset of BBS data from USA (840) for the specified years
```{r eval = FALSE}
# install.packages("pullBBS")
species.df <- pullBBS::pullBBS(year = year, country = 840, useCache = T)
```

Add BirdTree species names and eliminate records of unrecognised species
```{r eval = FALSE}
...
Repeate for the next year, etc. Once all of the data is downloaded, we can read in all the separate yearly BBS datasets and filter them
```{r eval = FALSE}
...
species.df = species.df %>%
  dplyr::filter(RPID == 101 & RunType == 1 & RouteTypeID == 1 & RouteTypeDetailId %in% c(1,2)) %>%
