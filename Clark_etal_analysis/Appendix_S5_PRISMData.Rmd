---
title: "Appendix S5 - Extracting PRISM Climate Data"
...
This appendix describes how to calculate mean PRISM climate variables for 40km-radius buffers around BBS sample sites. These matrices can be accessed in the `BBS.occurrences` library using `data('Bird.phy.distances')` and `data('Site.descriptors')`.  
...
We need to extract unique sample sites and convert these points to a spatial dataframe so that they can be intersected with the `prism` rasters
```{r eval = FALSE}
library(dplyr)
...
```{r eval = FALSE}
buffer <- 40000
points.ppt <- meanPRISM(points = points, points_spdf = points.spdf,
