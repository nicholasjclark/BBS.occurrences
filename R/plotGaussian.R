...
#'using model output from \code{lassoAbund}
#'
#'
#'
#'@param lassoAbund A model object returned from \code{lassoAbund_comm}
...
#'@details Species' pairwise influences on scaled abundances are extracted from
#'\code{lassoAbund$graph} and predicted
...
#'
#'@export
plotGaussian = function(lassoAbund, cutoff){
  ...
  sp.names <- rownames(lassoAbund$graph)
  ...
  return(gridExtra::grid.arrange(hab.plot, phy.plot, ncol = 1,
                                 heights = c(1, 1),
                                 left = grid::textGrob("Species' pairwise influence on scaled abundance",
                                                       rot = 90, vjust = 1.5)))
